...
namespace FASTSURVEY.Services.Tokens
{
    ...
    {
        Task<ServiceResult<TokenDto>> CriarAsync(
            int validadeMinutos = 10,
            CancellationToken ct = default
        );
        Task<ServiceResult<TokenDto>> ObterPorIdAsync(int id, CancellationToken ct = default);
        Task<ServiceResult<List<TokenDto>>> ListarAsync(
            bool apenasAtivos,
            int? pagina = null,
            int? tamanho = null,
            CancellationToken ct = default
        );
        Task<ServiceResult<ValidarTokenResponse>> ValidarAsync(
            string token,
            CancellationToken ct = default
        );
        Task<ServiceResult<TokenDto>> ProrrogarAsync(
            int tokenId,
            int minutos,
            CancellationToken ct = default
        );
