...
namespace FASTSURVEY.Services.Tokens
{
    public class TokenService : ITokenService
    {
        ...
        public async Task<ServiceResult<TokenDto>> CriarAsync(
            int validadeMinutos = 10,
            CancellationToken ct = default
        )
        {
            if (validadeMinutos <= 0)
                validadeMinutos = 10;

            // Gera token único (6 chars alfanuméricos) com algumas tentativas
            string novo;
            int tentativas = 0;
            do
            {
                if (tentativas++ > 10)
                    return ServiceResult<TokenDto>.Fail(
                        "TOKEN_GEN",
                        "Não foi possível gerar um token único."
                    );
                novo = GerarCodigo(6);
            ...
            {
                // Raro: colisão por concorrência
                return ServiceResult<TokenDto>.Fail(
                    "TOKEN_DUP",
                    "Colisão de token. Tente novamente."
                );
            }
        ...
        public async Task<ServiceResult<TokenDto>> ObterPorIdAsync(
            int id,
            CancellationToken ct = default
        )
        {
            var t = await _ctx.Set<TokenEntity>()
                .AsNoTracking()
                .FirstOrDefaultAsync(x => x.TokenId == id, ct);
            if (t is null)
                return ServiceResult<TokenDto>.Fail("NOT_FOUND", "Token não encontrado.");
            return ServiceResult<TokenDto>.Ok(Map(t));
        }

        public async Task<ServiceResult<List<TokenDto>>> ListarAsync(
            bool apenasAtivos,
            int? pagina = null,
            int? tamanho = null,
            CancellationToken ct = default
        )
        {
            ...
            if (apenasAtivos)
                q = q.Where(x => x.DataExpirado > DateTime.UtcNow);

            var lista = await q.OrderByDescending(x => x.DataRegistro)
                .Skip(skip)
                .Take(take)
                .ToListAsync(ct);
        ...
        public async Task<ServiceResult<ValidarTokenResponse>> ValidarAsync(
            string token,
            CancellationToken ct = default
        )
        {
            ...
            if (t is null)
                return ServiceResult<ValidarTokenResponse>.Ok(
                    new ValidarTokenResponse
                    {
                        Valido = false,
        ...
        public async Task<ServiceResult<TokenDto>> ProrrogarAsync(
            int tokenId,
            int minutos,
            CancellationToken ct = default
        )
        {
            if (minutos <= 0)
                return ServiceResult<TokenDto>.Fail("INVALID", "Minutos deve ser > 0.");

            var t = await _ctx.Set<TokenEntity>()
                .FirstOrDefaultAsync(x => x.TokenId == tokenId, ct);
            if (t is null)
                return ServiceResult<TokenDto>.Fail("NOT_FOUND", "Token não encontrado.");

            // se já expirou, começa a contar a partir de agora
        ...
        public async Task<ServiceResult<bool>> RevogarAsync(
            int tokenId,
            CancellationToken ct = default
        )
        {
            var t = await _ctx.Set<TokenEntity>()
                .FirstOrDefaultAsync(x => x.TokenId == tokenId, ct);
            if (t is null)
                return ServiceResult<bool>.Fail("NOT_FOUND", "Token não encontrado.");
        ...
        private static TokenDto Map(TokenEntity t) =>
            new()
            {
                TokenId = t.TokenId,
                Token = t.Token,
